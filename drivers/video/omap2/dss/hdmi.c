...
#if 1 //def CONFIG_OMAP_ENHANCEMENT_S3D
	...
	for (i = 0; i < CEA_MODEDB_SIZE; i++) {
		if (relaxed_fb_mode_is_equal(cea_modes + i, vm)) {
			*vm = cea_modes[i];
			if (check_only)
				return 1;
#if 0 //mo2sanghyun.lee 1080P 24hz test
                        if(vm->yres * vm->xres >= 1920*1080)
			    hdmi.cfg.cm.code = 32;
                        else
#endif                            
			hdmi.cfg.cm.code = i;
...
#if defined(CONFIG_P2_GAMMA) || defined(CONFIG_U2_GAMMA) || defined(CONFIG_CX2_GAMMA)
...
// LGE_CHANGE_S [sungho.jung@lge.com] 2012-02-20
	...
	1A-07 // 1B-01a // 1B-04 // 1B-05 for HDCP CTS */
