...
#ifdef CONFIG_MACH_LGE_CX2//hongkeon.kim 2012-0625 keylock command
...
static void touch_work_func_a(struct work_struct *work)
{
	...
	/* Button handle */
#ifdef CONFIG_MACH_LGE_CX2//hongkeon.kimj 2012-0625 keylock command
...
#ifdef CONFIG_MACH_LGE_CX2 //nthyunjin.yang 120625 for key lock test
	if(atcmd_keylock == 1)
		ts->ts_data.state = DO_NOT_ANYTHING;
#endif
...
abs_report:
	...
	case TOUCH_BUTTON_LOCK:
		if (unlikely(touch_debug_mask & DEBUG_BUTTON))
		TOUCH_INFO_MSG("Touch KEY[%d] is locked\n", ts->ts_data.curr_button.key_code); 	
		
	case TOUCH_ABS_LOCK:
		if (unlikely(touch_debug_mask & DEBUG_ABS))
		TOUCH_INFO_MSG("Touch pannel is pressed\n"); 	
		goto out;
		break;
	default:
		break;
